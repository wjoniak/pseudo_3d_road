...
road_width=texture2.get_width();road_height=texture2.get_height()
...
while True:
    ...
    #those are variables to draw a segmented curve, and it apply for hills too
    curve=0;edje=470;offset=0.1;k=0
    while wall_bottom>plane_center+10:
        #this is a simple algorithm to draw a curve
        #just try to draw a segmented curve with pygame draw.line
        #and try to port it here
        if wall_bottom>=edje:
             curve-=offset
        else:
           if wall_bottom>350:
        ...
        #to change the hill direction just change the sign before curve variable
        screen.blit(texture,(0,wall_bottom-curve),(0,floor_y,WIDTH,resolution))
        screen.blit(row_slice,(slice_x,wall_bottom-curve))
