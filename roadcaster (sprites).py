...
plant=pygame.image.load('plant-green.png').convert_alpha()
plant_scaled=plant
plant_size=64
plant_y_pos=100
plant_x_pos=[100,100]
...
while True:
    ...
    #while wall_bottom>plane_center+10:
        ...
        if wall_bottom==choosed_wall_bottom:
           #print(ray_x,ray_y,floor_x,floor_y,wall_bottom)
           #at 40 pixels is where the texture change color
           if floor_y>40:
              choosed_wall_bottom+=1#;print(choosed_wall_bottom)
              if choosed_wall_bottom>=HEIGHT:
                 choosed_wall_bottom=258#251
        #screen.blit(player_car,(240,320))
        #scaling sprite
        #if ray_y==plant_y_pos:
           plant_scaled_size=int(plant_size*(to_plane_dist/to_floor_dist))
           plant_scaled=pygame.transform.scale(plant, (plant_scaled_size,plant_scaled_size))
           plant_y_pos=wall_bottom-plant_scaled_size
           plant_x_pos=[slice_x,slice_x+slice_width-plant_scaled_size]
           #pygame.draw.line(screen, RED, (slice_x,choosed_wall_bottom), (slice_x+slice_width,choosed_wall_bottom), 1)
    screen.blit(plant_scaled,(plant_x_pos[0],plant_y_pos))
    screen.blit(plant_scaled,(plant_x_pos[1],plant_y_pos))
