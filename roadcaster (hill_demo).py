...
with open('hill.road', 'rb') as file:
     file_loader=pickle.Unpickler(file)
     curve=file_loader.load()
     
road_start=0   #this indicate the position on the road
road_index=0   #this is used to index all points on the curve
speed=5        #this is used to increment road_start
direction=1    #this is used to change the curve direction
curve_time=0   #how much time the curve will last
...
while True:
    ...
    if keys[K_UP]:
       ...
       if road_start>=len(curve)-230: #230 are number of road slices
          road_start=len(curve)-230
          curve_time+=1
    ...
    road_index=0
    wall_bottom_start=wall_bottom-resolution
    bottom_start_dy=(wall_bottom-resolution)-(wall_bottom_start+curve[road_start+road_index])
    old_y=1000;current_y=0
    while wall_bottom>plane_center+10:
        ...
        current_y=wall_bottom+curve[road_start+road_index]*direction+bottom_start_dy
        if current_y<old_y:
           screen.blit(texture,(0,current_y),(0,floor_y,WIDTH,resolution))
           screen.blit(row_slice,(slice_x,current_y))
        old_y=current_y
        if road_start+road_index<=len(curve)-2:
           dy=curve[road_start+road_index]*direction-curve[road_start+road_index+1]*direction
           if dy>0:
              i=0
              while i<dy:
                  i+=1
                  screen.blit(texture,(0,current_y-i),(0,floor_y,WIDTH,resolution))
                  screen.blit(row_slice,(slice_x,current_y-i))
